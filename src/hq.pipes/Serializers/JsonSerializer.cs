...
namespace hq.pipes.Serializers
{
    public class JsonSerializer : ISerializer
    {
        ...
        public Stream SerializeToStream<T>(T message)
        {
            ...
            using (var sw = new StreamWriter(s, Encoding.UTF8, 4096, leaveOpen: true))
            {
        ...
        {
            using (var sw = new StreamWriter(stream, Encoding.UTF8, 4096, leaveOpen: true))
            {
                using (var tw = new JsonTextWriter(sw))
                {
                    _serializer.Serialize(tw, message);
